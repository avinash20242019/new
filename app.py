...
with open("satellites.json", "r") as f:
    sats_raw = json.load(f)

# --- Fetch TLEs for all satellites ---
@st.cache_data(show_spinner=True, ttl=3600)
def get_tles(sats_dict):
    tle_map = {}
    for name, catnr in sats_dict.items():
        l1, l2 = fetch_tle(catnr)
        tle_map[name] = (name, l1, l2)  # store as tuple for propagate_positions
    return tle_map

with st.spinner("Fetching latest TLEs..."):
    tle_map_tuples = get_tles(sats_raw)
...
for idx, name in enumerate(names):
    ...
    initial_data.append(go.Scatter3d(
        x=[], y=[], z=[], mode="lines", name=f"{name} trail", line=dict(width=4), showlegend=False))
    initial_data.append(go.Scatter3d(
...
for k in range(num_frames):
    ...
    for name in names:
        ...
        frame_data.append(go.Scatter3d(
            x=trail[:,0], y=trail[:,1], z=trail[:,2], mode="lines",
